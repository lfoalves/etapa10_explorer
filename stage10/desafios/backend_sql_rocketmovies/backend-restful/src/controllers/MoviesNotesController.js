...
class MoviesNotesController {
  async create(request, response) {
    ...
    const userExists = await database.get('SELECT * FROM users WHERE id = ? LIMIT 1', [user_id]);
    ...
    if (rating) {
      if (rating <= 0 || rating > 5) throw new AppError('Nota do filme deve ser entre 1 e 5')
    }
  ...
  async show(request, reponse) {
    ...
    // console.log("ReUQETS", user_id, movie_id)
    ...
    const movieNote = await database.get(`
      SELECT * 
      FROM movie_notes
      WHERE id = (?)
      `, [movie_id]);
    ...
    if (!movieNote) throw new AppError('Não há notas sobre filmes cadastradas ainda por este usuário!')
    
    return reponse.json(movieNote)
  }
